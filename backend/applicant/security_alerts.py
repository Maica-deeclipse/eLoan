...
class SecurityAlertService:
    ...
    FAILURE_THRESHOLD = getattr(settings, 'SECURITY_ALERT_THRESHOLD', 3)  # Alert after N failures
    TIME_WINDOW = timedelta(hours=getattr(settings, 'SECURITY_ALERT_WINDOW_HOURS', 1))  # Within N hours
    ...
    def check_and_alert_repeated_failures(cls, user, application, failure_type):
        ...
        try:
            ...
            since = timezone.now() - cls.TIME_WINDOW
            ...
            logger.info(f"User {user.id} has {recent_failures} {failure_type} failures in last {cls.TIME_WINDOW}")

            # If threshold exceeded, create alert
            if recent_failures >= cls.FAILURE_THRESHOLD:
                logger.warning(f"ALERT: User {user.id} exceeded failure threshold ({recent_failures} >= {cls.FAILURE_THRESHOLD})")
                cls._create_security_alert(user, application, failure_type, recent_failures)
                return True
    ...
    @classmethod
    def _create_security_alert(cls, user, application, failure_type, count):
        """
        ...
        try:
            ...
            alert_message = cls._format_alert_message(user, application, failure_type, count)
            ...
            AuditLog.objects.create(
                ...
                severity='CRITICAL',
                success=False,
                failure_reason=f"{count} consecutive {failure_name} failures within {cls.TIME_WINDOW}",
                related_application=application
            )
    ...
    @classmethod
    def _format_alert_message(cls, user, application, failure_type, count):
        """
        ...
        message = (
            ...
            f"ALERT REASON:\n"
            f"User has failed {failure_desc} {count} times within the last {cls._format_time_window()}.\n\n"
    ...
    def _format_time_window(cls):
        ...
        """
        hours = int(cls.TIME_WINDOW.total_seconds() / 3600)
        if hours == 1:
            return "1 hour"
        else:
    ...
    def get_failure_statistics(cls, user, days=7):
        ...
        try:
            ...
            return {
                ...
                'is_high_risk': (face_failures + liveness_failures) >= (cls.FAILURE_THRESHOLD * 2)
            }
