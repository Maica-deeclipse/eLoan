...
class LivenessPhotosView(ApplicantBaseView):
    """
    POST /api/applicant/applications/<app_id>/liveness-photos/

    Performs liveness detection from multiple photos.
    Analyzes photos taken during different liveness challenges (blink, smile, turn head).
    This is simpler than video-based liveness and doesn't require audio permissions.

    Request:
        - photos: Multiple image files (JPEG, PNG)
    ...
    def post(self, request, app_id):
        ...
        if len(photos) < 2:
            return Response(
                {'error': 'At least 2 liveness photos are required'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        # Create directory for liveness photos
        full_dir = os.path.join(settings.MEDIA_ROOT, f"applicant/liveness/{application.id}")
        os.makedirs(full_dir, exist_ok=True)
        ...
        try:
            ...
            for i, photo in enumerate(photos):
                # Save photo to disk
                ext = os.path.splitext(photo.name)[1].lower()
                filename = f"liveness_photo_{i}_{uuid.uuid4().hex[:12]}{ext}"
                ...
                # Analyze this photo for liveness
                result = LivenessVerificationService.verify_liveness_for_application(
                    application_id=application.id,
            ...
            # Encrypt saved files after processing
            from .encryption_utils import FileEncryptionService
            for file_path in saved_files:
                encrypt_success, encrypt_error = FileEncryptionService.encrypt_file(file_path)
                if not encrypt_success:
                    logger.error(f"Failed to encrypt liveness photo {file_path}: {encrypt_error}")

            photos_analyzed = len(liveness_results)
            ...
            # Aggregate results - require majority of photos to pass
            passed_photos = sum(1 for r in liveness_results if r.get('is_live', False))
            avg_confidence = sum(r.get('confidence', 0) for r in liveness_results) / len(liveness_results)

            overall_passed = passed_photos >= (photos_analyzed * 0.6)  # 60% must pass

            response_data = {
                'method': 'photos',
                'photos_analyzed': photos_analyzed,
                'photos_passed': passed_photos,
                'pass_rate': f"{(passed_photos / photos_analyzed * 100):.1f}%",
                'average_confidence': round(avg_confidence, 2),
                'is_live': overall_passed,
                'status': 'Verified' if overall_passed else 'Failed',
            }
            ...
            if overall_passed:
                response_data['message'] = f"Liveness check passed! Analyzed {photos_analyzed} photos with {avg_confidence:.1f}% average confidence."
                return Response(response_data, status=status.HTTP_201_CREATED)
            else:
                response_data['message'] = f"Liveness check failed. Only {passed_photos}/{photos_analyzed} photos passed. Please try again with better lighting."
                response_data['error'] = 'Liveness verification failed'
                return Response(response_data, status=status.HTTP_422_UNPROCESSABLE_ENTITY)

        except Exception as e:
            logger.exception(f"Photo liveness check error: {str(e)}")
            return Response({
                'error': f'Photo processing error: {str(e)}',
                'message': 'An error occurred while processing the photos. Please try again.',
            }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
